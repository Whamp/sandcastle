...
describe("README first-run onboarding", () => {
  it("guides a brand-new user through the default GitHub Issues host-execution run", () => {
    ...
    expect(earlyReadme).toContain(
      "GitHub CLI authenticated with `gh auth login`, or a `GH_TOKEN`",
    );
    expect(earlyReadme).toContain(
      "agent credential such as `ANTHROPIC_API_KEY`",
    );
    ...
    expect(earlyReadme).toContain("npx tsx .sandcastle/main.ts");
    ...
    expect(earlyReadme).toContain("runs the agent on your host machine");
    expect(earlyReadme).toContain(
