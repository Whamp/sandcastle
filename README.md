...
Sandcastle helps you point an AI coding agent at a ready GitHub issue, run that agent against your repo, and record what happened. It is a TypeScript toolkit for turning issue-sized work into an agent run with a repeatable setup, safe git branch/worktree handling, and a clear outcome.

For a first run, use the generated GitHub Issues worker: you mark one open issue with `ready-for-agent`, Sandcastle picks it, runs the selected agent (Pi by default) on your machine, and then records whether the work landed, was blocked, produced a follow-up, or finished without a change.

Sandcastle can also be used as a lower-level library for custom flows and sandboxed execution. Those API and provider details are kept in the reference sections after the first-run tutorial.

## First successful run

This path uses the default `github-issues-coordinator` template generated by `sandcastle init`. It runs on your host machine by default, uses GitHub Issues as the task list, and is the fastest way to prove Sandcastle can complete one real loop in your repo.

### Prerequisites for the default path

- Node.js and npm. Install Node from [nodejs.org](https://nodejs.org/) if needed.
- Git with a clean local checkout of the repository you want the agent to edit. Install Git from [git-scm.com](https://git-scm.com/) if needed.
- GitHub CLI authenticated with `gh auth login` (recommended), or a `GH_TOKEN` environment variable exported in your shell with permission to read, comment on, label, and close issues in that repo.
- A selected agent credential such as `ANTHROPIC_API_KEY` for Claude Code/Pi agent execution.
- A GitHub repository with Issues enabled.
- One open GitHub issue that describes a small, safe first task for the agent.

### 1. Install Sandcastle

From the repository where you want the agent to work:

```bash
npm install @ai-hero/sandcastle
```

### 2. Generate the default worker

```bash
npx sandcastle init
```

Choose the default GitHub Issues coordinator template when prompted. It creates `.sandcastle/main.mts` unless your package.json has `type: module`, in which case it creates `.sandcastle/main.ts`. It also creates prompt files and `.sandcastle/.env.example`.

### 3. Create `.sandcastle/.env`

```bash
cp .sandcastle/.env.example .sandcastle/.env
```

`.sandcastle/.env` is for the selected agent credential that the generated Sandcastle run loads, not for authenticating the manual `gh issue` commands below. Edit `.sandcastle/.env` and add the credentials your generated file asks for. For a Claude Code/Pi setup this may include:

```bash
ANTHROPIC_API_KEY=sk-ant-...
```

Use `gh auth login` for GitHub access and confirm `gh issue list` works in the repo. If you use token auth instead, export `GH_TOKEN` in your shell before running `gh issue` commands or the generated worker:

```bash
export GH_TOKEN=ghp_...
```

If you want to use your Claude subscription instead of an API key, see [#191](https://github.com/mattpocock/sandcastle/issues/191).

### 4. Create or mark a ready issue

Create the `ready-for-agent` label once, then create a small first issue with that label:

```bash
gh label create ready-for-agent --description "Ready for Sandcastle agent work" --color 0E8A16 --force
gh issue create --title "Try Sandcastle on a small docs fix" --body "Make one small, verifiable improvement and run the project checks." --label ready-for-agent
```

Or label an existing open issue:

```bash
gh issue edit 123 --add-label ready-for-agent
```

Sandcastle only selects open issues that are ready for the agent. For your first run, keep the issue narrow and include the commands the agent should use to verify the work.

### 5. Run the generated worker

Run whichever file `sandcastle init` generated:

```bash
npx tsx .sandcastle/main.mts
# or, when package.json has type: module:
npx tsx .sandcastle/main.ts
```

### What success looks like

On a successful first run, Sandcastle:

1. selects one open GitHub issue labeled `ready-for-agent`;
2. adds a claim note to that issue so another worker does not pick it at the same time;
3. runs the selected agent on your host machine using the generated prompt and the selected issue as context;
4. lands work according to the generated flow, usually by committing or merging the agent's changes through Sandcastle's worktree-safe branch handling; and
5. closes the issue or records the task outcome, such as a blocking prerequisite, proposed follow-up, or no landed change.

The generated worker prints a summary such as `Closed GitHub Issue #123 after land on ...` when the issue is closed, or a different outcome message when the claim is released, a prerequisite is recorded, or a follow-up issue is created.

### If there is no ready issue

If the worker prints:

```text
No ready GitHub Issue Task found.
```

Sandcastle connected to GitHub but did not find an open issue it is allowed to run. Add the `ready-for-agent` label to an open issue and run the worker again:
...
The first-run tutorial above is the recommended starting point. The sections below keep the deeper provider, API, template, and CLI reference material for custom flows.
