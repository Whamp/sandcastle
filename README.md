...
For a first run, use the generated GitHub Issues worker: you mark one open issue with `ready-for-agent`, Sandcastle picks it, runs Claude Code on your machine, and then records whether the work landed, was blocked, produced a follow-up, or finished without a change.
...
- GitHub CLI authenticated with `gh auth login`, or a `GH_TOKEN` environment variable with permission to read, comment on, label, and close issues in that repo.
- An agent credential such as `ANTHROPIC_API_KEY` for Claude Code/Pi agent execution.
...
Choose the default GitHub Issues coordinator template when prompted. It creates `.sandcastle/main.mts` (or `.sandcastle/main.ts` in older generated projects), prompt files, and `.sandcastle/.env.example`.
...
Edit `.sandcastle/.env` and add the credentials your generated file asks for. For the default path this is typically:
...
If you use `gh auth login` instead of `GH_TOKEN`, keep the token line out of `.sandcastle/.env` and make sure `gh issue list` works in the repo. If you want to use your Claude subscription instead of an API key, see [#191](https://github.com/mattpocock/sandcastle/issues/191).

### 4. Create or mark a ready issue

Create a small first issue and label it `ready-for-agent`:
...
# or, in older generated projects:
...
3. runs the agent on your host machine using the generated prompt and the selected issue as context;
